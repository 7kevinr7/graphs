...
class WeightedGraphNode(BaseGraphNode):
    ...
    def insert_neighbor(self, neighbor, weight):
        ...
        raise NeighborAlreadyExistsError(f"Neighbor: {neighbor}: already present for vertex: {self.value}")
    ...
    def remove_neighbor(self, neighbor, weight=None):
        ...
        raise NeighborDoesNotExistError(f"Neighbor: {neighbor}: not present for vertex: {self.value}")
    ...
    def union(self, other):
        ...
        """
        if self != other:
            raise VertexOperationError(f"Vertex: {self.value} cannot be added to by vertex: {other.value}")
        
        if not self.multiple_edges:
            for key in self.neighbors:
                if key in other.neighbors:
                    if not other.multiple_edges:
                        self.neighbors[key] = min(self.neighbors[key], other.neighbors[key])
                    else:
                        self.neighbors[key] = min(self.neighbors[key], min(other.neighbors[key]))
    ...
    def intersection(self, other):
        ...
        """
        if self != other:
            raise VertexOperationError(f"Vertex: {self.value} cannot be intersected with vertex: {other.value}")
    ...
    def difference(self, other):
        ...
        """
        if self != other:
            raise VertexOperationError(f"Vertex: {self.value} cannot be subtracted by vertex: {other.value}")
